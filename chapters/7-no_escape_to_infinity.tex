...
\begin{lemma}\label{lem:nested-sweepout-small-intersection}
    ...
    \begin{proof}
        ...
        We are going to inductively push $\Omega'$ outside of $B_1'\cap U, B_2'\cap U ,\dots, B_N'\cap U$:
                \begin{figure}[htbp]
            \centering
            \includegraphics[width=0.8\textwidth]{figures/IMG_2675.jpg}
            \caption{Pushing $\Omega'$ outside of $B_1'\cap U$}
            \label{fig:2675}
        \end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=10cm,y=1cm]
    % B band (blue) - 先画色块
    \fill[blue!12] (0.18,-0.25) rectangle (0.42,0.25);
    \fill[blue!12] (0.60,-0.25) rectangle (0.86,0.25);
    
    % A band (red) disjoint from B
    \fill[red!10] (0.46,-0.15) rectangle (0.56,0.15);
    ...
    % I = [t0,t1] 
    \draw[ultra thick,blue!70] (0.58,0) -- (0.90,0);
    \fill (0.58,0) circle (0.4pt);
    \fill (0.90,0) circle (0.4pt);
    \draw (0.58,-0.05) node[below=2pt] {$t_0$};
    \draw (0.90,-0.05) node[below=2pt] {$t_1$};
    
    % I' inside B
    \draw[very thick,blue] (0.62,0) -- (0.86,0);
    \node[blue] at (0.74,0.3) {$I$};
    
    % Conditions with arrows
    \node[align=center] at (0.40,0.95) {$g(t_0)\le \varepsilon_0$};
    \draw[->,thick] (0.48,0.75) -- (0.58,0.08);
    
    \node[align=center] at (1.00,0.95) {$g(t_1)\ge \mathrm{Vol}(U)-\varepsilon_0$};
    \draw[->,thick] (0.92,0.75) -- (0.90,0.08);
    
    % Labels below axis
    \node[blue!60] at (0.20,-0.8) {$B=\{g\in(\varepsilon_0,\ \mathrm{Vol}(U)-\varepsilon_0)\}$};
    \node[red!60] at (0.75,-0.8) {$A=\{\mathcal H^n(\Gamma_t\cap M)\ge W_g(U)\}$};
  \end{tikzpicture}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/IMG_2676.PNG}
  \caption{Thin set $P$ removed and smoothed to $\overline{U}$ inside a fixed collar of $\partial U$.}
  \label{fig:PUbar}
\end{figure}
...
\begin{enumerate}
  ...
  that pushes $\partial_{\mathrm{rel}}\overline U$ outside $U$ through slices of area at most $3\mathbf{M}(\partial_IU)$, after absorbing the $O(\varepsilon_0^{\frac{n}{n+1}})$ error into the constant (again by choosing
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/IMG_2677.JPG}
  \caption{Cut–cap–insert at the slice $\overline\Gamma_{t_0}$. The hatched red part is the portion of the slice outside $\overline U$, bounded in area by $2\,\mathcal H^n(\partial_{\mathrm{rel}}U)$. Capping the outside within the collar and inserting a nested homotopy inside $\overline U$ enable a sweepout below $W_g(U)$.}
  \label{fig:cut-cap-insert}
\end{figure}

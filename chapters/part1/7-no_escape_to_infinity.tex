% !TEX root = ../../thesis.tex
\section{No Escape to Infinity}\label{ch:no-escape-to-infinity}
...
This section establishes a quantitative non-escape result: any good sweepout must have at least one slice carrying a definite amount of area inside $U$. The free-boundary setting introduces additional technical complications in the proof (particularly in handling boundary collars and relative isoperimetric estimates), but the fundamental difficulty remains the completeness of the ambient manifold.

The proof proceeds by contradiction: assuming every slice with large area has negligible intersection with $\overline{U}$, we use the nested sweepout structure from Section~\ref{ch:nested-sweepout} to construct a competing sweepout with strictly smaller width, contradicting the definition of $W_g(U)$. The precise statement is:
...
The first technical ingredient is a lemma that allows us to deform a set with small intersection with $U$ so that it avoids $U$ entirely, while controlling the area increase. Let $U$ be a good set.
\begin{lemma}\label{lem:nested-sweepout-small-intersection}
    ...
    \begin{proof}
        ...
        \end{equation}
        where $C(n)$ is the constant from Lemma~\ref{lem:morse-separate}. Using the coarea inequality (Lemma~\ref{lem:coarea}), we may choose a covering $\br{B_i'}$ of $U$ by $N$ balls of radius $r_i \in (r_0/2, r_0)$, so that         \begin{equation}\label{eq:area-of-Bi}
            ...
            \begin{tikzpicture}[scale=0.7]
              ...
              \begin{scope}
                ...
                \begin{scope}
                  ...
                  \fill[pattern=north east lines, pattern color=hatchcol]
                    (-2,-1) rectangle (12,7);
                \end{scope}
              \end{scope}
              ...
              % Circle B_i' (brown)
              \draw[thick, black] (4,3) circle (1.2);
              ...
              \begin{scope}
                ...
                \clip (-1,0) -- (-1,6) -- (3,6)
                  .. controls (3,5) and (3.5,4) .. (4,3)
                  .. controls (4.5,2) and (5,1) .. (5,0) -- cycle;
                \draw[ultra thick, hlcol] (4,3) circle (1.2);
              \end{scope}
              \node[black, above right] at (4.8,3.8) {$B_i'$};

              % Bottom line (partial M)
              \draw[very thick, black] (-1,0) -- (11,0);

              % Labels
              \node[bindB, font=\large] at (1.5,3.5) {$\Omega'$};
              \node[bindA, font=\large] at (7.5,3) {$\mathcal{U}$};
              \node[hlcol, above] at (1.5,6) {$\Omega'\setminus(B_i'\cap \mathcal{U})$};
              \node[black, right] at (11,-0.4) {\color{black}$\partial M$};
            \end{tikzpicture}
            \caption{Pushing $\Omega'$ outside of $B_1'\cap U$. The region $\Omega'$ (blue) overlaps with $\mathcal{U}$ (red) near the ball $B_i'$. The set $\Omega' \setminus (B_i' \cap \mathcal{U})$ is obtained by removing the portion of $\Omega'$ inside $B_i' \cap \mathcal{U}$.}
            \label{fig:2675}
        \end{figure}
...
\begin{lemma}[$\delta$-continuous perturbation]\label{lem:delta-cts-perturbation}
    ...
    \begin{proof}
        ...
        \end{equation*}
        Shrinking $\eta>0$ if necessary and using the coarea inequality (Lemma~\ref{lem:coarea}) and the small-volume choice from Lemma~\ref{lem:preliminary-modification}, we may assume
        \begin{equation*}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[y=1cm]
    ...
    \fill[fillB] (0.3,-0.4) rectangle (1.2,0.4);
    \fill[fillB] (2.0,-0.4) rectangle (3.2,0.4);
    \fill[fillB] (5.5,-0.4) rectangle (8.5,0.4);

    % A band (red, matching U color scheme)
    \fill[fillA] (3.8,-0.4) rectangle (4.8,0.4);
    ...
    % --- I = [t0,t1] ---
    \draw[line width=2.5pt, hlcol, opacity=0.5] (5.0,0) -- (9.0,0);
    ...
    \draw[line width=2pt, hlcol] (5.5,0) -- (8.5,0);
    \node[hlcol] at (7.0,0.55) {$I'$};
    ...
    % --- Legend ---
    \fill[fillB] (0.3,-2.5) rectangle (0.7,-2.2);
    \node[anchor=west, bindB, font=\small] at (0.8,-2.35)
      {$B=\{t: g(t)\in(\varepsilon_0,\,\mathrm{Vol}(U)-\varepsilon_0)\}$};
    \fill[fillA] (6.5,-2.5) rectangle (6.9,-2.2);
    \node[anchor=west, bindA, font=\small] at (7.0,-2.35)

...
The min-max theory of Li--Zhou~\cite{LZ17} produces a stationary varifold whose support is a minimal hypersurface, but does not by itself guarantee that $\partial\Sigma \neq \varnothing$: in general, the limit could be a closed minimal hypersurface in the interior of $M$ (see \cite[Remark~1.3]{LZ17}). Following the standard convention, a closed minimal hypersurface is regarded as a (degenerate) free boundary minimal hypersurface with $\partial\Sigma = \varnothing$; the conditions $\partial\Sigma \subset \partial M$ and the orthogonal meeting condition in Theorem~\ref{thm:main} are then vacuously satisfied. When $M$ is a compact domain in $\mathbb{R}^{n+1}$, the closed case cannot occur since $\mathbb{R}^{n+1}$ admits no closed minimal hypersurface.
\end{remark}

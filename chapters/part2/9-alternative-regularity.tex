...
In the classical Almgren--Pitts min-max theory~\cite{Alm65, Pit81}, the regularity of the min-max hypersurface is established through two key ingredients: the \emph{almost minimizing} property, which is obtained from a combinatorial selection argument, and the Schoen--Simon regularity theory~\cite{SS81} for stable minimal hypersurfaces. The framework operates with integral currents, so integrality of the limit object is built into the construction from the outset.

Chodosh--Liokumovich--Spolaor~\cite{CLS22} introduced an alternative approach to min-max theory based on \emph{non-excessive sweepouts}. Rather than working with integral currents, their framework operates with varifolds and sweepouts by Caccioppoli sets. The non-excessive condition---roughly, that critical slices of the sweepout cannot be locally replaced by competitors of strictly smaller mass---provides structural control that replaces the almost minimizing property of Almgren--Pitts. This approach has led to striking applications, including generic regularity of min-max hypersurfaces in dimension~$8$ and a bound relating the Morse index to the number of non-area-minimizing singularities~\cite{CLS22}.

A key step in the non-excessive framework is the construction of \emph{optimal nested sweepouts}: optimal sweepouts whose sublevel sets are monotonically increasing. The existence of such sweepouts relies on the monotonisation technique of Chambers--Liokumovich~\cite{CL20}, which converts arbitrary sweepouts into nested ones with controlled mass increase. The nested sweepout machinery developed in Part~1 of this thesis provides exactly this input (see Proposition~\ref{prop:p2-onvp-existence}).

In this chapter, we explore an alternative path to regularity within the non-excessive framework: instead of the Schoen--Simon theory, we use Wickramasekera's regularity theorem~\cite{Wic14} for stable codimension~$1$ integral varifolds. Wickramasekera's theorem gives optimal regularity (empty singular set for $2 \leq n \leq 6$) under two hypotheses: (i)~the varifold is a stable integral varifold, and (ii)~it satisfies the \emph{$\alpha$-structural hypothesis} for some $\alpha \in (0, 1/2)$---no singular point has a neighbourhood in which the support consists of three or more embedded $C^{1,\alpha}$ hypersurfaces-with-boundary meeting only along their common boundary. Combining these tools, we obtain:

\noindent\textbf{Main result} (informal)\textbf{.}
\textit{Let $(M^{n+1}, g)$ be a closed Riemannian manifold with $2 \leq n \leq 6$. Then the non-excessive min-max construction, using nested sweepouts as input and Wickramasekera's theorem for regularity, produces a smooth, closed, embedded minimal hypersurface, provided no mass cancellation occurs.}

The proof proceeds in three steps. First, we show that in the absence of mass cancellation, the limit varifold is integral: since the varifold mass equals the width, the approximating Caccioppoli sets converge with no loss of perimeter, and the De~Lellis--Tasnady integrality criterion~\cite[Proposition~A.1]{DLT13} applies. Second, we verify the $\alpha$-structural hypothesis by analyzing blow-up cones at potential singular points and using the non-excessive condition to rule out configurations where three or more sheets meet. Third, the non-excessive structure also ensures stability (or one-sided minimization), so Wickramasekera's theorem~\cite{Wic14} (or the one-sided regularity of Liu~\cite{Liu19}) yields smoothness.

\medskip
\noindent\textbf{The mass cancellation problem.}
The mass cancellation case---where the current masses $\mathbf{M}(\Phi(x_i))$ converge to the width $W$ but $\mathbf{M}(\Phi(x_0)) < W$ at the limit parameter $x_0$---remains an open problem. In this situation, sheets of the Caccioppoli boundaries with opposite orientations cancel in the flat topology while contributing positively to varifold mass, and the De~Lellis--Tasnady criterion no longer applies. Establishing integrality of the limit varifold without the no-cancellation hypothesis appears to require fundamentally new ideas, and we formulate this as a conjecture (Conjecture~\ref{conj:mass-cancellation-integrality}).

\medskip
\noindent\textbf{Organization.}
Section~\ref{sec:p2-prelim} collects the basic definitions: Caccioppoli sets, varifolds, sweepouts, width, and the integrality criterion of De~Lellis--Tasnady.
Section~\ref{sec:non-excessive-prelim} reviews the non-excessive sweepout framework of~\cite{CLS22}, including the definitions of excessive intervals, non-excessive points, and homotopic minimizers. Section~\ref{sec:integrality} establishes integrality of the limit varifold without mass cancellation and discusses the mass cancellation case as an open problem. The final section verifies the $\alpha$-structural hypothesis via Wickramasekera's regularity theorem~\cite{Wic14} and completes the proof of the main theorem.

\section{Preliminaries}\label{sec:p2-prelim}

Throughout this chapter, $(M^{n+1}, g)$ denotes a smooth, closed (compact without boundary) Riemannian manifold with $2 \leq n \leq 6$. We write $\mathcal{H}^k$ for the $k$-dimensional Hausdorff measure and $\mathbf{M}(T)$ for the mass of a current $T$.

\subsection{Caccioppoli sets and varifolds}

A measurable subset $\Omega \subset M$ is a \emph{Caccioppoli set} (set of finite perimeter) if the distributional derivative $D\mathbf{1}_\Omega$ is a finite Radon measure. Its \emph{perimeter} in an open set $U$ is $\mathrm{Per}(\Omega, U) := |D\mathbf{1}_\Omega|(U)$, and its \emph{reduced boundary} $\partial^*\Omega$ is the set of points where $D\mathbf{1}_\Omega$ has an approximate unit normal. By De~Giorgi's structure theorem, $\mathrm{Per}(\Omega, U) = \mathcal{H}^n(\partial^*\Omega \cap U)$.

An \emph{$n$-varifold} on $M$ is a Radon measure on the Grassmannian bundle $\mathrm{Gr}_n(M)$. Each Caccioppoli set $\Omega$ induces a varifold $|\partial^*\Omega|$ by
\[
|\partial^*\Omega|(f) = \int_{\partial^*\Omega} f(x, T_x(\partial^*\Omega))\, d\mathcal{H}^n(x).
\]
A varifold $V$ is \emph{stationary} if $\delta V(X) := \int_{\mathrm{Gr}_n(M)} \operatorname{div}_S X \, dV = 0$ for every $X \in \mathfrak{X}_c(M)$. It is \emph{stable} if, in addition, the second variation is non-negative for all compactly supported normal deformations of $\mathrm{reg}\, V$.

\subsection{Sweepouts and width}

\begin{definition}[Sweepout]\label{def:p2-sweepout}
A \emph{sweepout} of $M$ is a family of Caccioppoli sets $\{\Omega_t\}_{t \in [0,1]}$ satisfying:
\begin{enumerate}[label=\textup{(\roman*)}]
    \item $\Omega_0 = \varnothing$ and $\Omega_1 = M$;
    \item $t \mapsto \mathbf{1}_{\Omega_t}$ is continuous in $L^1(M)$;
    \item $\sup_{t} \mathrm{Per}(\Omega_t) < \infty$;
    \item (no mass concentration) $\lim_{r \to 0} \sup_{t} \mathrm{Per}(\Omega_t, B_r(x)) = 0$ for every $x \in M$.
\end{enumerate}
The \emph{height} of the sweepout is $\mathcal{F}(\{\Omega_t\}) := \sup_{t} \mathrm{Per}(\Omega_t)$.
\end{definition}

\begin{definition}[Width]\label{def:p2-width}
The \emph{width} of $M$ is
$W = W(M) := \inf\bigl\{\mathcal{F}(\{\Omega_t\}) : \{\Omega_t\} \text{ is a sweepout of } M\bigr\}$.
\end{definition}

\begin{definition}[Nested sweepout]\label{def:p2-nested}
A sweepout $\{\Omega_t\}$ is \emph{nested} if $\Omega_s \subset \Omega_t$ for all $0 \leq s \leq t \leq 1$.
\end{definition}

The monotonisation technique of Chambers--Liokumovich~\cite{CL20} shows that every sweepout can be approximated by a nested sweepout with arbitrarily small mass increase. Combined with an Arzel\`a--Ascoli argument and volume reparametrisation, this yields:

\begin{proposition}[Existence of optimal nested sweepouts~{\cite[Theorem~1.5]{CLS22}, \cite{CL20}}]\label{prop:p2-onvp-existence}
There exists an optimal nested sweepout $\{\Omega_t\}$ of $M$ with $\mathcal{F}(\{\Omega_t\}) = W$.
\end{proposition}

\subsection{Integrality criterion}

The following proposition from the appendix of De~Lellis--Tasnady~\cite{DLT13} gives a criterion for varifold convergence of Caccioppoli boundaries:
...
\begin{proposition}[Varifolds vs.\ Caccioppoli set limits~{\cite[Proposition~A.1]{DLT13}}]\label{prop:p2-DLT-integrality}
    ...
    \begin{enumerate}[label=\textup{(\roman*)}]
        \item $D\mathbf{1}_{\Omega^k} \to D\mathbf{1}_\Omega$ in the sense of measures in $U$;
...
\begin{itemize}
    ...
    \item \emph{nested} if $\Omega(x_1) \subset \Omega(x_2)$ for all $0\leq x_1 \leq x_2\leq 1$ (Definition~\ref{def:p2-nested});
...
\section{Integrality without Mass Cancellation}\label{sec:integrality}
...
\[
    \mathbf{M}(\phi(t^*)) = W.
\]
Since the sweepout $\phi$ is continuous in the flat norm topology, and flat convergence implies $L^1$ convergence of characteristic functions, we have $\mathbb{1}_{\Omega_i} \to \mathbb{1}_{\Omega(t^*)}$ in $L^1(M)$. By uniqueness of BV limits, the Caccioppoli set $\Omega$ from Step~1 equals $\Omega(t^*)$.
...
\textbf{Step 3: Application of Proposition~\ref{prop:p2-DLT-integrality}.} By
Proposition~\ref{prop:p2-DLT-integrality} (De~Lellis--Tasnady, Proposition A.1), since we have both
...
\begin{proof}
If $V$ is integral, then by Proposition~\ref{prop:p2-DLT-integrality}, there exists a Caccioppoli set $\Omega$ such that
\[
...
Let $(M^{n+1}, g)$ be a closed Riemannian manifold with $2 \leq n \leq 6$. By Proposition~\ref{prop:p2-onvp-existence}, there exists an optimal nested sweepout of $M$. Applying the non-excessive tightening procedure of
Chodosh--Liokumovich--Spolaor~\cite{CLS22} to this nested sweepout yields a non-excessive sweepout

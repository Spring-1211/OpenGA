...
This dichotomy governs the integrality of the limit varifold:
...
When mass cancellation occurs ($\mathbf{M}(\Phi(x_0)) < W$), we still have $\Omega(x_i) \to \Omega(x_0)$ in $L^1$, but the perimeter convergence condition in Proposition~\ref{prop:p2-DLT-integrality} fails: the mass defect $W - \mathbf{M}(\Phi(x_0)) > 0$ is carried by sheets of $\partial^*\Omega(x_i)$ with opposite orientations that cancel in the flat topology while contributing positively to varifold mass, and the integrality of $V$ is not guaranteed. This case remains open (Conjecture~\ref{conj:mass-cancellation-integrality}).
...
The set $\mathfrak{h}_{\mathrm{nm}}(V)$ consists of points where the varifold fails to be one-sided minimizing in every neighborhood. A key consequence of~\cite[Proposition~3.1]{CLS22} is that for non-excessive sweepouts without mass cancellation, this set is at most finite. In particular, the limit varifold is one-sided homotopic minimizing---hence stable---at all but finitely many points. This will be used in Section~\ref{sec:alpha-verification} to verify the $\alpha$-structural hypothesis required by Wickramasekera's regularity theorem.

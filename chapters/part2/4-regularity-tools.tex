...
\begin{itemize}
    ...
    \item \textbf{The $\alpha$-structural hypothesis.} If the hypothesis fails at a singular point $Z$, the tangent cone consists of $N$ half-hyperplanes meeting along a common edge. The fold case $N = 2$ is excluded by elliptic regularity: the two tangential $C^{1,\alpha}$ sheets join to form a smooth hypersurface, contradicting $Z \in \sing V$ (Lemma~\ref{lem:wedge}). Therefore $N \geq 3$, and the pigeonhole principle produces an adjacent pair with angle $< \pi$; a chord-beats-arc replacement then yields a local area saving that, transferred to the approximating Caccioppoli sets, contradicts the min-max width (Theorem~\ref{thm:alpha-hypo}).
\end{itemize}

...
\end{enumerate}
Under either condition, $\sing V = \varnothing$ for $2 \leq n \leq 6$.
...
\begin{theorem}[Wickramasekera~{\cite{Wic14}}]\label{thm:wickramasekera}
    ...
    then $\sing V = \varnothing$, i.e., $\spt\|V\|$ is a smooth embedded minimal hypersurface.
\end{theorem}
...
\begin{itemize}
    ...
    \item \textbf{Integrality.} Integrality is established independently of the regularity theory: in the no-mass-cancellation case by the de~Lellis--Tasnady criterion, and in the mass-cancellation case by a graphical decomposition argument that decomposes the approximating boundaries into smooth sheets (Theorem~\ref{thm:integrality}). Neither nestedness nor the non-excessive condition is used.

    \item \textbf{The $\alpha$-structural hypothesis.} If the hypothesis fails at a singular point $Z$, the tangent cone consists of $N$ half-hyperplanes meeting along a common edge. The fold case $N = 2$ is excluded by~\cite[Remark~(3) following~($\mathcal{S}3$)]{Wic14}: for stable codimension~$1$ integral varifolds, the $\alpha$-structural hypothesis is equivalent to requiring $N \geq 3$, and flat tangent cones (with any multiplicity) are handled by the Sheeting Theorem (Lemma~\ref{lem:wedge}). Therefore $N \geq 3$, and the pigeonhole principle produces an adjacent pair with angle $< \pi$; a chord-beats-arc replacement then yields a local area saving that, transferred to the approximating Caccioppoli sets, contradicts the min-max width (Theorem~\ref{thm:alpha-hypo}).
\end{itemize}

...
\begin{itemize}
    ...
    \item \textbf{The $\alpha$-structural hypothesis.} In the no-mass-cancellation case, $V = |\partial^*\Omega(x_0)|$ is the boundary of a Caccioppoli set, which constrains tangent cones to wedges and folds---configurations incompatible with the $\alpha$-structural failure. In the mass-cancellation case, nestedness constrains the possible junction structures, and any remaining junctions are ruled out by area-minimizing replacements (Theorem~\ref{thm:alpha-hypo}).
\end{itemize}

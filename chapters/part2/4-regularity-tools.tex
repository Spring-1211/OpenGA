...
\begin{itemize}
    ...
    \item \textbf{The $\alpha$-structural hypothesis.} If the hypothesis fails at a singular point $Z$, the tangent cone consists of $N$ half-hyperplanes meeting along a common edge. The fold case $N = 2$ is excluded: if $m_1 = 1$, Allard's regularity theorem gives $Z \in \reg V$; if $m_1 \geq 2$, the support is locally a smooth hypersurface with higher multiplicity and no branching structure, so the $\alpha$-structural hypothesis does not fail (Lemma~\ref{lem:wedge}). Therefore $N \geq 3$, and the pigeonhole principle produces an adjacent pair with angle $< \pi$; a chord-beats-arc replacement then yields a local area saving that, transferred to the approximating Caccioppoli sets, contradicts the min-max width (Theorem~\ref{thm:alpha-hypo}).
\end{itemize}

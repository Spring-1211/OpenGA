...
\begin{definition}[One-sided minimizers~{\cite[Definition~1.10]{CLS22}}]\label{def:one-sided-perimeter-min}
Let $(M^{n+1}, g)$ be a Riemannian manifold, $\Omega$ a Caccioppoli set, and $U \subset M$ open.
We say $\Omega$ is \emph{locally one-sided inner area-minimizing} in~$U$ if for every $A \Subset U$ and every Caccioppoli set $F$ with $F \mathbin{\triangle} \Omega \subset A$ and $F \subset \Omega$,
\[
    \mathrm{Per}(\Omega, A) \leq \mathrm{Per}(F, A).
\]
Similarly, $\Omega$ is \emph{locally one-sided outer area-minimizing} in $U$ if the same holds for every $F \supset \Omega$ with $F \mathbin{\triangle} \Omega \subset A$.
\end{definition}
...
\begin{proposition}[Mass ratio lower bound for one-sided minimizers]\label{prop:mass-ratio-lower-bound}
Let $(M^{n+1}, g)$ be a Riemannian manifold, $U \subset M$ open, and $\Omega$ a Caccioppoli set that is locally one-sided (inner or outer) area-minimizing in $U$ (Definition~\ref{def:one-sided-perimeter-min}). Then for every $q \in \partial^*\Omega \cap U$ and every sufficiently small $\rho > 0$,
\begin{equation}\label{eq:mass-ratio-lower-bound}
    \mathrm{Per}(\Omega, B_\rho(q)) \geq c\,\rho^n,
\end{equation}
where $c > 0$ depends only on $n$ and $(M,g)$.
\end{proposition}

\begin{proof}
We follow the isoperimetric argument of~\cite[\S0.3, p.~16]{Lin85}.
Assume $\Omega$ is locally one-sided outer area-minimizing (the inner case is symmetric).
Let $q \in \partial^*\Omega \cap U$ and choose $\rho > 0$ small enough that $B_\rho(q) \Subset U$.
Set $f(\rho) := \mathrm{Per}(\Omega, B_\rho(q))$.
By the coarea formula, for a.e.\ $\rho$ the slice $\partial^*\Omega \cap \partial B_\rho(q)$ is an $(n-1)$-rectifiable cycle with
\[
    \mathcal{H}^{n-1}(\partial^*\Omega \cap \partial B_\rho(q)) = f'(\rho).
\]
By Proposition~\ref{prop:caccioppoli-competitor}, there exists a Caccioppoli set $F$ with $F \supset \Omega$ and $F \mathbin{\triangle} \Omega \subset B_\rho(q) \Subset U$, so $F$ is an admissible competitor in Definition~\ref{def:one-sided-perimeter-min}.
By one-sided outer area-minimality and~\eqref{eq:caccioppoli-competitor},
\[
    f(\rho) = \mathrm{Per}(\Omega, B_\rho(q)) \leq \mathrm{Per}(F, B_\rho(q)) \leq C\,f'(\rho)^{n/(n-1)},
\]
...
\begin{lemma}[Density from perimeter minimality on one side]\label{lem:one-sided-density}
...
Suppose that for some fixed $R > 0$ and $p \in \spt\|V\|$, each $\Omega_i$ is locally one-sided (inner or outer) area-minimizing in $B_R(p)$ (Definition~\ref{def:one-sided-perimeter-min}; the side may vary with~$i$).
Then $\Theta(\|V\|, p) > 0$.
\end{lemma}

\begin{proof}
Assume $\Omega_i$ is locally one-sided outer area-minimizing in $B_R(p)$ (the inner case is symmetric).
By Proposition~\ref{prop:mass-ratio-lower-bound}, for every $q \in \partial^*\Omega_i \cap B_{R/2}(p)$ and $0 < r \leq R/4$,

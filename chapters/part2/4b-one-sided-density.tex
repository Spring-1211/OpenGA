...
\subsection{Proof strategy: sweepout-wide \texorpdfstring{$I$}{I}-replacement (in progress)}\label{subsec:proof-strategy}
...
\paragraph{Notation.}
Let $\Phi = \{\Omega_t\}_{t\in[0,1]}$ be a non-excessive ONVP sweepout with width $W$. After passing to a subsequence we fix $t_i \nearrow t_0 \in \mathfrak{m}(\Phi)$ such that $|\partial^*\Omega_{t_i}| \to V$ as varifolds; by nestedness $\Omega_{t_1} \supset \Omega_{t_2} \supset \cdots$. Throughout this subsection $p \in \spt\|V\|$ is fixed.
...
\paragraph{Quantitative consequences.}
...
\paragraph{The proposed $I$-replacement.}
...
\paragraph{Heuristic justification.}
Inside the ball:
...
\paragraph{Conclusion modulo Sub-claim~\ref{subclaim:interior-interpolation}.}

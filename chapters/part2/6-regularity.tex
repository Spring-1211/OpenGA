...
Let $V$ be a stationary integral $n$-varifold in $(M^{n+1}, g)$ and $Z \in \sing V$ a point at which the $\alpha$-structural hypothesis \textup{(}Definition~\textup{\ref{defn:alpha-structural})} fails. Then any tangent cone $C$ of $V$ at $Z$ consists of $N \geq 3$ half-hyperplanes $P_1^+, \ldots, P_N^+$ meeting along a common $(n-1)$-dimensional edge $L$. In particular, there exist two adjacent half-hyperplanes meeting at angle $\theta < \pi$.
\end{lemma}
...
We claim $N \geq 3$. Suppose $N = 2$. The balance condition $m_1 \nu_1 + m_2 \nu_2 = 0$ with $|\nu_j| = 1$ forces $\nu_1 = -\nu_2$ and $m_1 = m_2$, so the two half-hyperplanes form a complete hyperplane $H$ and $C = m_1|H|$ is flat.

If $m_1 = 1$, then $C = |H|$ is a multiplicity-one flat plane. By Allard's regularity theorem~\cite[Theorem~24.2]{Sim84}, $Z \in \reg V$, contradicting $Z \in \sing V$.

If $m_1 \geq 2$, then $C = m_1|H|$ is a flat hyperplane with even multiplicity. The two half-hyperplanes form a single complete hyperplane, so $\spt\|V\|$ near $Z$ is locally a smooth hypersurface carrying higher multiplicity---there is no branching or junction structure of distinct sheets meeting from different directions along a common boundary. Therefore the $\alpha$-structural hypothesis does not fail at such points; they fall within the scope of Wickramasekera's regularity theorem~\cite{Wic14}, which handles high-multiplicity flat tangent cones internally.

Therefore $N \geq 3$. The $N$ half-hyperplanes partition the $2$-plane perpendicular to $L$ into $N$ sectors with angles $\theta_1, \ldots, \theta_N$ summing to $2\pi$. By the pigeonhole principle, $\min_j \theta_j \leq 2\pi/N < \pi$.
\end{proof}
...
\medskip\noindent\textbf{Step~1: Area improvement via chord-beats-arc.}
...
By varifold convergence $|\partial^*\Omega(x_i)| \to V$, for all $i$ sufficiently large $\partial^*\Omega(x_i) \cap B_r(Z)$ inherits a near-junction structure. Since each $\partial^*\Omega(x_i)$ is the boundary of a Caccioppoli set (smooth for $n \leq 6$ by Proposition~\ref{prop:p2-federer-regularity}), the chord competitor transfers via Caccioppoli surgery on $\Omega(x_i)$: excise the region bounded by the narrowest adjacent pair and the chord surface, yielding a modified set $\Omega(x_i)_r$ with
\[

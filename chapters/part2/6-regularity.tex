...
We claim $N \geq 3$. Suppose $N = 2$. The balance condition $m_1 \nu_1 + m_2 \nu_2 = 0$ with $|\nu_j| = 1$ forces $\nu_1 = -\nu_2$ and $m_1 = m_2$, so the two half-hyperplanes form a complete hyperplane $H$ and the tangent cone $C = m_1|H|$ is flat. The two $C^{1,\alpha}$ sheets from the $\alpha$-structural failure are therefore tangent to $H$ along $\Gamma$, and they join to form a $C^{1,\alpha}$ hypersurface \emph{without boundary} in $B_\rho(Z)$. Since $V$ is stationary, this hypersurface satisfies the minimal surface equation weakly; elliptic regularity upgrades $C^{1,\alpha}$ to $C^\infty$, so $Z \in \reg V$, contradicting $Z \in \sing V$.

...
\begin{proof}
By Proposition~\ref{thm:CLS-stationary}, $V$ is stationary; by Proposition~\ref{prop:integrality-nested}, $V$ is integral. By Wickramasekera's theorem (Theorem~\ref{thm:wickramasekera}), it suffices to verify that $V$ is stable and satisfies the $\alpha$-structural hypothesis.
...
Blow up $V$ at $Z$: any tangent cone $C$ is a stationary integral cone in $\mathbb{R}^{n+1}$. Unlike the no-mass-cancellation case (where $V = |\partial^*\Omega(x_0)|$ constrains tangent cones to wedges and folds), the mass cancellation varifold $V$ is not \emph{a priori} the boundary of a single Caccioppoli set. The $\alpha$-structural failure gives $C$ a junction structure, and three cases arise:
\begin{itemize}
    \item \emph{Wedge ($\theta < \pi$):} $C = |H_1| + |H_2|$, two half-hyperplanes meeting along an $(n-1)$-edge at angle $\theta < \pi$. The chord-beats-arc replacement (as in the proof of Theorem~\ref{thm:alpha-hypo}, Case~1a) saves area $\delta(\theta) r^n > 0$ in $B_r$.

    \item \emph{Fold ($\theta = \pi$):} $C = 2|H|$, a single half-hyperplane with multiplicity~$2$. Collapsing the fold saves area $(\omega_n/2) r^n$ (as in Case~1b of Theorem~\ref{thm:alpha-hypo}).

    \item \emph{Higher-order junction ($N \geq 3$ sheets):} In the mass cancellation case, tangent cones need not be boundaries, so junctions of $N \geq 3$ half-hyperplanes meeting along a common $(n-1)$-edge can occur. However, codimension-$1$ junctions in $\mathbb{R}^{n+1}$ with $N \geq 3$ are never area-minimizing: an area-minimizing replacement in $B_r$ (which exists by~\cite[Theorem~5.1.6]{Fed69}) has strictly less mass than $C \mres B_r$, yielding a deficit $\delta r^n > 0$.
\end{itemize}

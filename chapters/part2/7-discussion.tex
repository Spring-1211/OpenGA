...
\begin{proof}
The proof combines three ingredients: the monotonicity formula for stationary varifolds, graphical decomposition under small tilt-excess, and the sphere constraint to exclude partial sheets.
...
The \emph{tilt-excess} of a varifold $W$ in $B_r(p)$ relative to $P$ is
\[
    E(W, B_r(p), P) := r^{-n} \int_{B_r(p) \times G(n,n+1)} |S - P|^2 \, dW(x, S).
\]
Since $V$ has tangent plane $P$ at $p$, standard varifold theory~\cite[Section~22]{Sim84} gives $E(V, B_r(p), P) \to 0$ as $r \to 0$. The varifold convergence $|\partial^*\Omega_i| \to V$ implies $E(|\partial^*\Omega_i|, B_r(p), P) \to E(V, B_r(p), P)$ for each generic $r$ (with $\|V\|(\partial B_r(p)) = 0$). Combining: for every $\delta > 0$, there exist $r_1 > 0$ and $i_1$ such that for $r < r_1$ and $i > i_1$,
\[
    E(|\partial^*\Omega_i|, B_r(p), P) < \delta.
\]

\medskip\noindent\textbf{Step~3: Graphical decomposition.}
Since each $\partial^*\Omega_i$ is a smooth embedded hypersurface (using $n \leq 6$), we apply the following standard result (cf.~\cite[Chapter~7]{Sim84} and~\cite[Lemma~2.1]{Wic14}): there exists $\delta_0 = \delta_0(n) > 0$ such that if $\Sigma$ is a smooth closed embedded hypersurface in $B_R(0) \subset \mathbb{R}^{n+1}$ satisfying
...
Since $V$ is stationary (Proposition~\ref{thm:CLS-stationary}), Allard's rectifiability theorem~\cite[Theorem~5.5(1)]{All72} gives that $V = \theta \, \mathcal{H}^n \mres \Sigma$ for an $\mathcal{H}^n$-rectifiable set $\Sigma$ and a function $\theta \colon \Sigma \to (0, \infty)$. The monotonicity formula~\cite[Section~17]{Sim84} ensures $\theta(p) > 0$ at every $p \in \spt\|V\|$.

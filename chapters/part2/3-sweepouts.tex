...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \node[bindA, font=\small] at (2.3, 2.0) {$\partial^*\!\Omega_1$};
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \node[bindA, font=\small] at (1.2, 0) {$\partial^*\!\Xi_1$};
      ...
      \draw[bindA!50, thin]
        ...
        .. controls (-0.45, 0.25) and (-0.45, -0.25) ..
        ({2.5*cos(205)}, {2.5*sin(205)});
      \node[bindA!50, font=\small] at (-0.7, 2.8) {$\partial^*\!\Xi_0$};
      ...
      \node at (0, -3.0) {(b)};
    \end{tikzpicture}
    \caption{Cross-sections of non-trivial multiple junctions (see the $\alpha$-structure hypothesis in Section~\ref{sec:regularity}). The red branches form $\spt\|V\|$; the lighter curves are the approximating Caccioppoli boundaries $\partial^*\Omega(x_i)$, with shading indicating the nested Caccioppoli sets. \textbf{(a)}~No mass cancellation: $\mathbf{M}(\Phi(x_0)) = W$, so the varifold mass equals the perimeter. \textbf{(b)}~Mass cancellation: $\mathbf{M}(\Phi(x_0)) < W$. The dashed blue branch represents perimeter that cancels in the sense of currents, illustrating the discrepancy between varifold mass $\|V\|(B_r)$ and perimeter $\mathrm{Per}(\Omega(x_0), B_r)$.}

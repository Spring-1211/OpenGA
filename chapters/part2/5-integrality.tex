...
\begin{lemma}[Rectifiability and density lower bound]\label{lem:density-lower-bound}
Let $(M^{n+1}, g)$ be a closed Riemannian manifold with $n \geq 2$, and let $\Phi$ be an optimal nested \textup{(}ONVP\textup{)} sweepout. Let $V$ be the varifold limit of $|\partial^*\Omega(x_i)|$ along a min-max sequence $x_i \to x_0 \in \mathfrak{m}(\Phi)$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
    \item $V$ is rectifiable;
    \item $\Theta(\|V\|, p) \geq 1$ for every $p \in \spt\|V\|$.
\end{enumerate}
\end{lemma}

\begin{proof}
\textbf{Part~(i).} $V$ is stationary (Proposition~\ref{thm:CLS-stationary}) with $\|V\|(M) = W < \infty$. For any Radon measure $\mu$ on $M$ with $\mu(M) < \infty$, the upper $n$-density $\Theta^{n*}(\mu, p) := \limsup_{r \to 0} \omega_n^{-1} r^{-n}\mu(B_r(p))$ satisfies $\Theta^{n*}(\mu, p) > 0$ for $\mu$-a.e.\ $p$~\cite[Theorem~3.5(1)]{Sim84}\xinze{Simon states this for Radon measures on $\mathbb{R}^{n+1}$. For a closed Riemannian manifold $(M, g)$, work in normal coordinates: the metric balls $B_r^g(p)$ satisfy $B_{r(1-Cr^2)}^e(p) \subset B_r^g(p) \subset B_{r(1+Cr^2)}^e(p)$ for $r < \mathrm{inj}(M)$, so the $n$-densities with respect to $g$-balls and Euclidean balls agree.}. Since $V$ is stationary, the monotonicity formula (Proposition~\ref{prop:p2-monotonicity}) ensures that the density $\Theta(\|V\|, p) = \lim_{r \to 0} \omega_n^{-1} r^{-n} \|V\|(B_r(p))$ exists at every $p \in M$; in particular $\Theta(\|V\|, p) = \Theta^{n*}(\|V\|, p) > 0$ for $\|V\|$-a.e.\ $p$. By Allard's rectifiability theorem (Proposition~\ref{prop:allard-rectifiability}), $V$ is rectifiable.

\medskip
\textbf{Part~(ii).} Once $V$ is rectifiable, Theorem~\ref{thm:integrality} (proved below via the sheet decomposition) gives that $V$ is integral. For integral stationary varifolds, $\Theta(\|V\|, p) \geq 1$ for every $p \in \spt\|V\|$\xinze{If $V$ is integral and $p \in \spt\|V\|$, every tangent cone $C$ at $p$ is a non-zero stationary integral cone in $\mathbb{R}^{n+1}$, so $\Theta(\|C\|, 0) \geq 1$. Since $\Theta(\|V\|, p) = \Theta(\|C\|, 0)$ by the definition of tangent cone, $\Theta(\|V\|, p) \geq 1$.}.
\end{proof}

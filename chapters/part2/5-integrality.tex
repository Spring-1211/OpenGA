...
The idea is a dichotomy on the almost-minimality of the boundaries $\partial^*\Omega(x_i)$. If infinitely many $\Omega(x_i)$ are one-sided almost area-minimizing near~$p$, then the mass ratio lower bound for almost-minimizers (Proposition~\ref{prop:mass-ratio-lower-bound}) passes to the limit via the Portmanteau theorem, giving $\Theta(\|V\|, p) > 0$. Otherwise, each $\Omega(x_i)$ admits a one-sided competitor that saves $\varepsilon_0\,\eta_i^n$ in perimeter at some scale~$\eta_i$; if $\eta_i \to 0$, then $\Omega(x_i)$ is almost-minimizing at all larger scales, and an ODE integration argument again yields a mass ratio lower bound.
...
\begin{proof}
Since $V$ is stationary, the density $\Theta(\|V\|, p)$ exists at every point (Proposition~\ref{prop:p2-monotonicity}).
Fix $p \in \spt\|V\|$ and $R > 0$ smaller than the injectivity radius of $(M,g)$.
Let $\varepsilon_0 = \varepsilon_0(n, M, g) > 0$ be the threshold from Proposition~\ref{prop:mass-ratio-lower-bound}.
We distinguish two cases.
\begin{list}{}{\leftmargin=1.5em \parsep=\parskip \topsep=\smallskipamount \itemsep=\medskipamount}
\item \textbf{Case~1:} \emph{$\Omega(x_i)$ is locally one-sided $\varepsilon_0$-almost area-minimizing in $B_R(p)$ for infinitely many~$i$.}
...
\item \textbf{Case~2:} \emph{$\Omega(x_i)$ is not locally one-sided $\varepsilon_0$-almost area-minimizing in $B_R(p)$ for all large~$i$.}

This is the negation of Case~1. Define
\begin{gather*}
...
Since Case~2 is the negation of Case~1, $\bar{\eta}_i > 0$ for all large~$i$.
...
\begin{list}{}{\leftmargin=1.5em \parsep=\parskip \topsep=\smallskipamount \itemsep=\medskipamount}
\item \emph{Sub-case~(a): $\limsup_{i \to \infty} \bar{\eta}_i > 0$.}
...
\item \emph{Sub-case~(b): $\bar{\eta}_i \to 0$.}
...
\end{list}% end Sub-cases (a)/(b)
\end{list}% end Cases 1/2

\medskip
In both cases $\Theta(\|V\|, p) > 0$ for every $p \in \spt\|V\|$, and $V$ is rectifiable by Allard's theorem (Proposition~\ref{prop:allard-rectifiability}).
\end{proof}

...
The sheet decomposition (Lemma~\ref{lem:sheet-decomposition}) and integrality (Theorem~\ref{thm:integrality}) do not require nestedness or the non-excessive hypothesis: they hold for any sequence of Caccioppoli sets whose associated varifolds converge to a stationary rectifiable varifold, provided $2 \leq n \leq 6$. (Our sheet decomposition is closely related to Wickramasekera's Sheeting Theorem~\cite[Theorem~3.3]{Wic14}, but applies to the \emph{approximating} varifolds $|\partial^*\Omega_i|$ rather than the limit, avoiding the need to assume integrality a priori.) Similarly, the $\alpha$-structural verification (Theorem~\ref{thm:alpha-hypo}) uses only the Caccioppoli structure and the min-max width. The non-excessive hypothesis enters only in the stability step (Proposition~\ref{prop:stability}), which invokes~\cite[Proposition~3.1]{CLS22}; that proof uses nestedness in an essential way. Nestedness is also used in the parity analysis of Section~\ref{sec:multiplicity-one}.
\end{remark}

...
If the limit varifold $V$ is stationary, then $\Theta(\|V\|, p) > 0$ by the monotonicity formula applied directly to~$V$, so no $I$-replacement is needed.
Assume therefore that $V$ is non-stationary.
We construct an explicit $I$-replacement on $I = (x_i, x_0]$ in three steps.
\begin{list}{}{\leftmargin=1.5em \parsep=\parskip \topsep=\smallskipamount \itemsep=\medskipamount}
\item \emph{Step~1 (area-decreasing flow with uniform rate).}
Since $\delta V \neq 0$, there exists a smooth vector field~$X$ (independent of~$i$) with $\delta V(X) < 0$.
By varifold convergence $|\partial^*\Omega(x_i)| \to V$, we have $\delta|\partial^*\Omega(x_i)|(X) \to \delta V(X)$, so for all large~$i$,
\[
    \delta|\partial^*\Omega(x_i)|(X) \leq \tfrac{1}{2}\,\delta V(X) =: -c_0 < 0.
\]
Let $\{\Phi_t\}_{t \geq 0}$ denote the diffeomorphisms generated by~$X$.
The map $t \mapsto \mathrm{Per}(\Phi_t(\Omega(x_i)))$ has derivative $\delta|\partial^*\Omega(x_i)|(X) \leq -c_0$ at $t = 0$, so for a fixed small $t_0 > 0$ (independent of~$i$),
\[
    \mathrm{Per}\bigl(\Phi_t(\Omega(x_i))\bigr)
    \leq W - \tfrac{c_0}{2}\,t
    \quad\text{for all } t \in [0, t_0].
\]
Set $\Omega'_i := \Phi_{t_0}(\Omega(x_i))$, so $\mathrm{Per}(\Omega'_i) \leq W - c_1$ where $c_1 := c_0 t_0/2 > 0$ is independent of~$i$.

\item \emph{Step~2 (restoring nesting and interpolation).}
Since the sweepout is nested and $x_i < x_0$, we have $\Omega(x_i) \subset \Omega(x_0)$.
The flow may push part of $\Omega(x_i)$ outside $\Omega(x_0)$, so we restore nesting by setting
\[
    \hat\Omega_i := \Omega'_i \cap \Omega(x_0) \;\subset\; \Omega(x_0).
\]
We claim $\mathrm{Per}(\hat\Omega_i) < W$ for $i$ large.
By the lattice property of Caccioppoli sets,
\begin{equation}\label{eq:lattice}
    \mathrm{Per}(\Omega'_i \cap \Omega(x_0)) + \mathrm{Per}(\Omega'_i \cup \Omega(x_0))
    \leq \mathrm{Per}(\Omega'_i) + \mathrm{Per}(\Omega(x_0)).
\end{equation}
Since $\Omega(x_i) \subset \Omega(x_0)$ and the flow displacement is bounded,
$\mathrm{Vol}(\Omega'_i \setminus \Omega(x_0)) \leq \mathrm{Vol}(\Omega'_i \mathbin{\triangle} \Omega(x_i)) \to 0$,
so $\Omega'_i \cup \Omega(x_0) \to \Omega(x_0)$ in $L^1$.
By lower semicontinuity of perimeter, $\liminf_i \mathrm{Per}(\Omega'_i \cup \Omega(x_0)) \geq \mathrm{Per}(\Omega(x_0))$.
Combining with~\eqref{eq:lattice}:
\[
    \limsup_{i \to \infty}\mathrm{Per}(\hat\Omega_i)
    \leq \limsup_i \mathrm{Per}(\Omega'_i)
    \leq W - c_1 < W,
\]
so $\mathrm{Per}(\hat\Omega_i) < W$ for all large~$i$.
We now verify the hypotheses of Lemma~\ref{lem:interpolation} with $\Omega_0 := \hat\Omega_i$ and $\Omega_1 := \Omega(x_0)$:
\begin{itemize}
\item \emph{Nesting:} $\hat\Omega_i \subset \Omega(x_0)$ by construction.
\item \emph{Perimeter bound:} $\mathrm{Per}(\hat\Omega_i),\, \mathrm{Per}(\Omega(x_0)) < W$, so both are $\leq L := W$.
\item \emph{Volume smallness:} $\mathrm{Vol}(\Omega(x_0) \setminus \hat\Omega_i) \leq \mathrm{Vol}(\Omega(x_0) \setminus \Omega(x_i)) + \mathrm{Vol}(\Omega(x_i) \mathbin{\triangle} \Omega'_i) \to 0$.
\end{itemize}
Set $W_0 := \max\{\mathrm{Per}(\hat\Omega_i),\, \mathrm{Per}(\Omega(x_0))\} < W$ and $\varepsilon := (W - W_0)/2 > 0$.
Lemma~\ref{lem:interpolation} provides $\delta > 0$; for $i$ large, $\mathrm{Vol}(\Omega(x_0) \setminus \hat\Omega_i) \leq \delta$, yielding a nested $\mathcal{F}$-continuous family $\{\Omega_t^{(2)}\}_{t \in [0,1]}$ from $\hat\Omega_i$ to $\Omega(x_0)$ with
$\mathrm{Per}(\Omega_t^{(2)}) \leq (W + W_0)/2 < W$.

\item \emph{Step~3 (concatenation).}
Define $\Phi^I$ on $\bar{I} = [x_i, x_0]$ in two phases.
On $[x_i, m]$ (where $m := (x_i+x_0)/2$), reparametrise the path $t \mapsto \Phi_t(\Omega(x_i)) \cap \Omega(x_0)$ for $t \in [0, t_0]$; this is $\mathcal{F}$-continuous and runs from $\Omega(x_i)$ to~$\hat\Omega_i$.
On $[m, x_0]$, reparametrise the interpolation family from Step~2.
Then $\Phi^I(x_i) = \partial\Omega(x_i)$, $\Phi^I(x_0) = \partial\Omega(x_0)$, and $\Phi^I$ is $\mathcal{F}$-continuous on~$\bar{I}$.
For every $x \in I = (x_i, x_0]$: in the flow phase ($x \in (x_i, m]$), the lattice bound gives $\mathrm{Per}(\Phi_t(\Omega(x_i)) \cap \Omega(x_0)) \leq \mathrm{Per}(\Phi_t(\Omega(x_i))) + o_i(1) \leq W - c_0 t/2 + o_i(1) < W$ for $t > 0$ and $i$ large; in the interpolation phase, $\mathrm{Per} < W$ by Step~2.
Hence $\limsup_{I \ni y \to x}\mathbf{M}(\Phi^I(y)) < W$ for all $x \in I$, so $\Phi^I$ is an $I$-replacement (Definition~\ref{def:excessive-interval}), making $x_0$ left excessive—a contradiction.
\end{list}% end Steps
\end{list}% end Cases 2a/2b

\medskip
In Cases~1 and~2b we obtain an $I$-replacement contradicting non-excessiveness; in Cases~2a and the stationary sub-case of~2b, the monotonicity formula gives $\Theta(\|V\|, p) > 0$ directly. In all cases $\Theta(\|V\|, p) > 0$ for every $p \in \spt\|V\|$, and $V$ is rectifiable by Allard's theorem (Proposition~\ref{prop:allard-rectifiability}).
\end{proof}

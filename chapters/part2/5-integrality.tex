...
\end{equation}
By varifold convergence $|\partial^*\Omega_i| \to V$, the monotonicity formula (Proposition~\ref{prop:p2-monotonicity}), and the vanishing of tilt-excess (Lemma~\ref{lem:excess-vanishing}), for any $\varepsilon > 0$ and all $i$ sufficiently large, the surface $\partial^*\Omega_i \cap B_{2r}(p)$ has mass at most $C\omega_n r^n$ and tilt-excess less than $\varepsilon$. It therefore decomposes into finitely many Lipschitz graphs $\Sigma_i^1, \ldots, \Sigma_i^{k_i}$ over $P$ with Lipschitz constant $\mathrm{Lip}(u_i^j) < C\varepsilon^{1/4}$ (the tilt-excess $< \varepsilon$ bounds the $L^2$ average of $|T_x - P|^2$; separating $\partial^*\Omega_i$ into the region where $|T_x - P|^2 \geq \sqrt{\varepsilon}$ and its complement, the former has $\mathcal{H}^n$-measure $\leq \sqrt{\varepsilon}\,(2r)^n$ while on the latter $|T_x - P| < \varepsilon^{1/4}$, giving $\mathrm{Lip} < C\varepsilon^{1/4}$).

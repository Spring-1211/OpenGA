...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{A Caccioppoli set $\Omega$ (shaded cap) on a closed manifold $M \cong S^{n+1}$, with reduced boundary $\partial^*\!\Omega$ (solid curve). When no mass cancellation occurs, the limit varifold equals $V = |\partial^*\!\Omega(x_0)|$, the integral varifold induced by $\partial^*\!\Omega(x_0)$ with multiplicity~$1$.}

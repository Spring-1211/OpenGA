...
\xinze{The following proposition works only for the case of non-mass cancellation. It is actually covered by Lemma 5.4}
...
\xinze{is this gives us any regularity?}
...
\]
The mass defect $W - \mathrm{Per}(\Omega(x_0)) > 0$ is carried by sheets of $\partial^*\Omega(x_i)$ that converge in pairs, cancelling in the flat topology but contributing positively to varifold mass. In particular, $V \neq |\partial^*\Omega(x_0)|$, and the integrality of $V$ does not follow from Proposition~\ref{prop:p2-DLT-integrality}. Instead, we use the nestedness of the sweepout to decompose the approximating boundaries into graphical sheets and show that the density is a positive integer at $\mathcal{H}^n$-a.e.\ point.
\xinze{This is not precise, rectifiability and integrality are two seperate lemmas. The former one does not use rectifiability.}
The limit varifold $V$ is rectifiable (Lemma~\ref{lem:density-lower-bound}). The main tool is the following graphical decomposition lemma.
...
\begin{proof}
Since $V$ is stationary, the density $\Theta(\|V\|, p)$ exists at every point (Proposition~\ref{prop:p2-monotonicity}). We argue by contradiction: suppose there exists $p \in \spt\|V\|$ with $\Theta(\|V\|, p) = 0$.
...
The coarea formula gives $f_i'(\rho) = \mathcal{H}^n(\partial B_\rho(p) \cap S_i)$ for a.e.\ $\rho$, where $S_i = \Omega(x_i)^{(1)}$ in the nearly-empty case and $S_i = \Omega(x_i)^{(0)}$ in the nearly-full case. The relative isoperimetric inequality (Proposition~\ref{prop:relative-isoperimetric}) gives
...
\xinze{should have density positive?}

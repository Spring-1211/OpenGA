...
\begin{proof}
By Allard's rectifiability theorem (Proposition~\ref{prop:allard-rectifiability}), it suffices to show that $\Theta(\|V\|, p) > 0$ for $\|V\|$-a.e.\ $p$. Since $V$ is stationary, the density $\Theta(\|V\|, p)$ exists at every point (Proposition~\ref{prop:p2-monotonicity}). We argue by contradiction: suppose $\|V\|(\{p \in \spt\|V\| : \Theta(\|V\|, p) = 0\}) > 0$. Since $\Phi$ is a non-excessive ONVP sweepout, by~\cite[Proposition~3.1]{CLS22} the set $\mathfrak{h}_{\mathrm{nm}}(V)$ of non-homotopic-minimizing points is at most finite (Definition~\ref{def:hnm}), hence $\|V\|$-null\xinze{Each singleton has $\|V\|$-measure zero: $\|V\|(\{q\}) \leq \|V\|(B_r(q)) \leq C r^n \to 0$ by the monotonicity formula. So a finite set is $\|V\|$-null.}. Therefore there exists $p \in \spt\|V\| \setminus \mathfrak{h}_{\mathrm{nm}}(V)$ with $\Theta(\|V\|, p) = 0$.

...
This is the negation of Case~2b(i). For each large~$i$, there exist $(q_i, \eta_i)$ with $B_{\eta_i}(q_i) \subset B_R(p)$ and a Caccioppoli set $F_i$ with $F_i \mathbin{\triangle} \Omega(x_i) \subset B_{\eta_i}(q_i)$ such that
\[
    \mathrm{Per}(\Omega(x_i), B_{\eta_i}(q_i)) > \mathrm{Per}(F_i, B_{\eta_i}(q_i)) + \varepsilon_0\,\eta_i^n
    \geq \varepsilon_0\,\eta_i^n.
\]
We show $\Theta(\|V\|, p) > 0$ by distinguishing two sub-cases according to the scale~$\eta_i$.

\emph{Sub-case~(a): $\limsup_{i \to \infty} \eta_i > 0$.}
Passing to a subsequence, $\eta_i \geq c > 0$ for all~$i$.
Since $B_{\eta_i}(q_i) \subset B_R(p)$,
\[
    \|V_i\|(B_R(p)) = \mathrm{Per}(\Omega(x_i),\, B_R(p))
    \geq \mathrm{Per}(\Omega(x_i),\, B_{\eta_i}(q_i))
    > \varepsilon_0\,c^n.
\]
...
\emph{Sub-case~(b): $\eta_i \to 0$.}
Passing to a subsequence, $\eta_i \to 0$.
Pick $q_i \in \partial^*\Omega(x_i) \cap B_{R/4}(p)$ (non-empty for large~$i$ since $p \in \spt\|V\|$) and set $f_i(\eta) := \mathrm{Per}(\Omega(x_i), B_\eta(q_i))$.
We show the absorption step in the proof of Proposition~\ref{prop:mass-ratio-lower-bound} holds at every scale, whether or not the one-sided almost-minimality is available.

Fix a.e.\ $\eta \in (0, R/4)$.
By Proposition~\ref{prop:caccioppoli-competitor}, there exists a Caccioppoli set~$F$ with $F \subset \Omega(x_i)$ (resp.\ $F \supset \Omega(x_i)$), $F \mathbin{\triangle} \Omega(x_i) \subset B_\eta(q_i)$, and
\[
    \mathrm{Per}(F, B_\eta(q_i)) \leq C\,f_i'(\eta)^{n/(n-1)},
\]
where $C = C(n, M, g)$ and $f_i'(\eta) = \mathcal{H}^{n-1}(\partial^*\Omega(x_i) \cap \partial B_\eta(q_i))$ by the coarea formula.
There are two cases:
\begin{enumerate}[label=\textup{(\alph*)}]
    \item \emph{Almost-minimality holds at $(q_i, \eta)$}: $f_i(\eta) \leq \mathrm{Per}(F, B_\eta(q_i)) + \varepsilon_0\,\eta^n \leq C\,f_i'(\eta)^{n/(n-1)} + \varepsilon_0\,\eta^n$.
    \item \emph{Almost-minimality fails at $(q_i, \eta)$}: $f_i(\eta) > \mathrm{Per}(F, B_\eta(q_i)) + \varepsilon_0\,\eta^n \geq \varepsilon_0\,\eta^n$.
\end{enumerate}
Since $q_i \in \partial^*\Omega(x_i)$, the density-$1$ condition gives $f_i(\eta) \geq (\omega_n/2)\,\eta^n$ for all sufficiently small~$\eta$.
With $\varepsilon_0 = \omega_n/4$ as in Proposition~\ref{prop:mass-ratio-lower-bound}, case~(b) gives $f_i(\eta) \geq \varepsilon_0\,\eta^n$, so in particular $\varepsilon_0\,\eta^n \leq f_i(\eta)$.
In both cases, for all small enough~$\eta$ we have $\varepsilon_0\,\eta^n \leq f_i(\eta)/2$, so
\[
    f_i(\eta)/2 \leq C\,f_i'(\eta)^{n/(n-1)}
\]
for a.e.\ small~$\eta$ (in case~(a) by subtracting the error; in case~(b) because $f_i(\eta) \leq C\,f_i'(\eta)^{n/(n-1)} + f_i(\eta)$ is trivial, and the sharper bound follows since $\mathrm{Per}(F) \geq 0$ forces $f_i(\eta) \leq C\,f_i'(\eta)^{n/(n-1)} + f_i(\eta) - \varepsilon_0\,\eta^n$, i.e.\ $\varepsilon_0\,\eta^n \leq C\,f_i'(\eta)^{n/(n-1)}$, and $f_i(\eta)/2 \leq f_i(\eta) - \varepsilon_0\,\eta^n \leq C\,f_i'(\eta)^{n/(n-1)}$ once $\varepsilon_0\,\eta^n \leq f_i(\eta)/2$).
Integrating as in Proposition~\ref{prop:mass-ratio-lower-bound} yields $f_i(\eta) \geq c\,\eta^n$ for $\eta \in (0, R/4)$.
The Portmanteau argument of Lemma~\ref{lem:one-sided-density} then gives $\Theta(\|V\|, p) > 0$.

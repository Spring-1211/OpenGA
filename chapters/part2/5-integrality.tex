...
The sheet decomposition and integrality do not require nestedness or the non-excessive hypothesis. They hold for any sequence of Caccioppoli sets whose associated varifolds converge to a stationary rectifiable varifold, provided $2 \leq n \leq 6$ (so that Federer regularity gives smooth approximating boundaries). Similarly, the $\alpha$-structural verification (Theorem~\ref{thm:alpha-hypo}) uses only the Caccioppoli structure and the min-max width, not nestedness or non-excessiveness. The non-excessive hypothesis enters only in the stability step (Proposition~\ref{prop:stability}), which invokes~\cite[Proposition~3.1]{CLS22}; that proof uses nestedness of the sweepout in an essential way (the monotonicity $\Omega(x_1) \subset \Omega(x_2)$ ensures that parameter sets of the form $\{x : \Omega(x) \cap R \neq \varnothing\}$ are intervals, enabling the replacement construction). Extending this to non-nested sweepouts remains an open question. Nestedness is also used in the parity analysis of Section~\ref{sec:multiplicity-one}.
\end{remark}

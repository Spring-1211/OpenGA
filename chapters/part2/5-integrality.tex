...
\begin{proof}
By Allard's rectifiability theorem (Proposition~\ref{prop:allard-rectifiability}), it suffices to show that $\Theta(\|V\|, p) > 0$ for $\|V\|$-a.e.\ $p$. Since $V$ is stationary, the density $\Theta(\|V\|, p)$ exists at every point (Proposition~\ref{prop:p2-monotonicity}). We argue by contradiction: suppose $\Theta(\|V\|, p) = 0$ for some $p \in \spt\|V\|$.
...
By Step~2, replacing $\Omega(x_i)$ by $M \setminus \Omega(x_i)$ if necessary (which swaps inner and outer but preserves the reduced boundary), we may assume
\begin{equation}\label{eq:volume-small}
    V_i(r) := \mathcal{L}^{n+1}(\Omega(x_i) \cap B_r(p)) = o(r^{n+1}).
\end{equation}

\emph{Finding a good radius.}
The coarea formula for the distance function $d_p = \mathrm{dist}(\cdot, p)$ gives
\[
    V_i'(\rho) = \mathcal{H}^n(\partial B_\rho(p) \cap \Omega(x_i)^{(1)}) \quad \text{for a.e.\ } \rho,
\]
and the relative isoperimetric inequality in $B_\rho(p)$ gives
\begin{equation}\label{eq:isoperimetric-lower}
    \mathrm{Per}(\Omega(x_i), B_\rho(p)) \geq c_n\, V_i(\rho)^{n/(n+1)}
\end{equation}
whenever $0 < V_i(\rho) \leq \frac{1}{2}\mathcal{L}^{n+1}(B_\rho(p))$, where $c_n > 0$ depends only on $(M, g, n)$.

We claim that there exists $\rho_i \in (0, r)$ with $V_i(\rho_i) > 0$ and
\begin{equation}\label{eq:good-radius}
    V_i'(\rho_i) < c_n\, V_i(\rho_i)^{n/(n+1)} \leq \mathrm{Per}(\Omega(x_i), B_{\rho_i}(p)).
\end{equation}
Indeed, $p \in \spt\|V\|$ gives $\|V\|(B_{r/2}(p)) > 0$, so
$\mathrm{Per}(\Omega(x_i), B_{r/2}(p)) > 0$ for $i$ large,
forcing $V_i(\rho) > 0$ for all $\rho \geq r/2$.
If $V_i'(\rho) \geq c_n\, V_i(\rho)^{n/(n+1)}$ for a.e.\
$\rho \in (r/2, r)$, set $\varphi = V_i^{1/(n+1)}$ and integrate:
\[
    \varphi'(\rho) = \frac{V_i'(\rho)}{(n{+}1)\,V_i(\rho)^{n/(n+1)}} \geq \frac{c_n}{n{+}1} \quad \text{a.e.\ on } (r/2, r),
\]
so $V_i(r)^{1/(n+1)} \geq V_i(r/2)^{1/(n+1)} + c_n r / (2(n{+}1))$, giving
$V_i(r) \geq c_0\, r^{n+1}$ for a constant $c_0 = c_0(n, M) > 0$.
...
\[
    E_i := \Omega(x_i) \setminus \overline{B}_{\rho_i}(p).
\]
Then $E_i \subset \Omega(x_i)$, $E_i \mathbin{\triangle} \Omega(x_i) = \Omega(x_i) \cap \overline{B}_{\rho_i}(p) \subset B_r(p)$, and for a.e.\ $\rho_i$ (chosen so that $\|V\|(\partial B_{\rho_i}(p)) = 0$ and $\mathcal{H}^{n-1}(\partial^*\Omega(x_i) \cap \partial B_{\rho_i}(p)) = 0$):
\begin{align}
    \mathrm{Per}(E_i, B_r(p))
    &= \mathrm{Per}(\Omega(x_i),\, B_r(p) \setminus \overline{B}_{\rho_i})
       + \mathcal{H}^n(\partial B_{\rho_i}(p) \cap \Omega(x_i)^{(1)}), \label{eq:per-replacement} \\
    \mathrm{Per}(\Omega(x_i), B_r(p))
    &= \mathrm{Per}(\Omega(x_i),\, B_r(p) \setminus \overline{B}_{\rho_i})
       + \mathrm{Per}(\Omega(x_i), B_{\rho_i}(p)). \label{eq:per-original}
\end{align}
Subtracting~\eqref{eq:per-original} from~\eqref{eq:per-replacement} and applying~\eqref{eq:good-radius}:
\begin{equation}\label{eq:per-decrease}
    \mathrm{Per}(E_i, B_r(p)) - \mathrm{Per}(\Omega(x_i), B_r(p))
    = V_i'(\rho_i) - \mathrm{Per}(\Omega(x_i), B_{\rho_i}(p)) < 0.
\end{equation}
Since $E_i = \Omega(x_i)$ outside $B_r(p)$, the total perimeter satisfies
\begin{align*}
    \mathrm{Per}(E_i, M)
    &= \mathrm{Per}(\Omega(x_i), M)
       + \bigl[\mathrm{Per}(E_i, B_r(p)) - \mathrm{Per}(\Omega(x_i), B_r(p))\bigr] \\
    &< \mathrm{Per}(\Omega(x_i), M).
\end{align*}

\emph{Continuous deformation.}
The family $\{E_i^s\}_{s \in [0, \rho_i]}$ defined by $E_i^s := \Omega(x_i) \setminus \overline{B}_s(p)$ is continuous in $L^1(M)$: $\mathcal{L}^{n+1}(E_i^s \mathbin{\triangle} E_i^{s'}) = \mathcal{L}^{n+1}(\Omega(x_i) \cap (B_{\max(s,s')} \setminus B_{\min(s,s')})) \to 0$ as $s' \to s$. Moreover, $E_i^0 = \Omega(x_i)$, $E_i^{\rho_i} = E_i$, and $E_i^s \subset \Omega(x_i)$ for every $s$.

\emph{Contradiction.}
Since $\Phi$ is a non-excessive ONVP sweepout, by~\cite[Proposition~3.1]{CLS22} the set $\mathfrak{h}_{\mathrm{nm}}(V)$ of non-homotopic-minimizing points is at most finite (Definition~\ref{def:hnm}). Now suppose $p \in \spt\|V\| \setminus \mathfrak{h}_{\mathrm{nm}}(V)$ with $\Theta(\|V\|, p) = 0$. Since $p \notin \mathfrak{h}_{\mathrm{nm}}(V)$, there exists $r_p > 0$ such that for all $i$ sufficiently large, $\partial^*\Omega(x_i)$ is one-sided homotopic minimizing in $B_{r_p}(p)$\xinze{CLS22 Prop~3.1 shows $\mathfrak{h}_{\mathrm{nm}}(V)$ is finite via the sweepout: for $p \notin \mathfrak{h}_{\mathrm{nm}}(V)$, non-excessiveness prevents the slices $\Omega(x_i)$ from being improved by a local homotopic deformation in $B_{r_p}(p)$, for all $i$ large. The precise mechanism: if the slices could be improved, one could build an $I$-replacement family contradicting non-excessiveness of $\Phi$.}. By nestedness, the same side (inner or outer) is small for all $i$ large\xinze{In the nearly-empty case ($V_i(r) = o(r^{n+1})$), inner fails. In the nearly-full case ($\mathcal{L}^{n+1}(B_r \setminus \Omega(x_i)) = o(r^{n+1})$), outer fails. By nestedness, $x_i \leq x_j \Rightarrow \Omega(x_i) \subset \Omega(x_j)$, so the same side is small for ALL $i$ large.}. Taking $r < r_p$ in the construction above, the replacement $E_i$ satisfies $E_i \subset \Omega(x_i)$ (resp.\ $E_i \supset \Omega(x_i)$), $\mathrm{Per}(E_i, B_r(p)) < \mathrm{Per}(\Omega(x_i), B_r(p))$, and is connected to $\Omega(x_i)$ by a continuous inner (resp.\ outer) deformation. This contradicts the one-sided homotopic minimality of $\partial^*\Omega(x_i)$ in $B_{r_p}(p)$ (Definition~\ref{def:homotopic-minimizer}).

Therefore $\Theta(\|V\|, p) > 0$ for every $p \in \spt\|V\| \setminus \mathfrak{h}_{\mathrm{nm}}(V)$. Since $\mathfrak{h}_{\mathrm{nm}}(V)$ is finite and $\|V\|(\{q\}) = 0$ for each $q$ (by the monotonicity formula: $\|V\|(B_r(q)) \leq C r^n \to 0$), we conclude $\Theta(\|V\|, p) > 0$ for $\|V\|$-a.e.\ $p$.

\medskip
We conclude that $\Theta(\|V\|, p) > 0$ for $\|V\|$-a.e.\ $p$. By Allard's rectifiability theorem, $V$ is rectifiable.
\end{proof}

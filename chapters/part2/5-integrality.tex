...
Let $(M^{n+1}, g)$ be a closed Riemannian manifold with $n \geq 2$, and let $\Phi$ be an optimal nested \textup{(}ONVP\textup{)} sweepout. Let $V$ be the varifold limit of $|\partial^*\Omega(x_i)|$ along a min-max sequence $x_i \to x_0 \in \mathfrak{m}(\Phi)$. Then $\Theta(\|V\|, p) > 0$ for every $p \in \spt\|V\|$. In particular, $V$ is rectifiable by Allard's theorem \textup{(Proposition~\ref{prop:allard-rectifiability})}.
\end{lemma}

\begin{proof}
Fix $p \in \spt\|V\|$ and $r \in (0, \mathrm{inj}(M)/4)$. Since $\|V\|(B_{r/4}(p)) > 0$, varifold convergence gives $\|\partial^*\Omega_i\|(B_{r/4}(p)) > 0$ for all $i$ sufficiently large, so there exists $q_i \in \spt\|D\chi_{\Omega_i}\| \cap B_{r/4}(p)$. The relative isoperimetric inequality on $(M, g)$ provides a constant $c_0 = c_0(M, g) > 0$ such that
\[
    \mathrm{Per}(\Omega_i, B_{r/2}(q_i)) \geq c_0\, r^n
\]
for every Caccioppoli set $\Omega_i$ and every $q_i \in \spt\|D\chi_{\Omega_i}\|$\xinze{At $q \in \spt|D\chi_\Omega|$, both $\Omega$ and $M \setminus \Omega$ have positive $(n{+}1)$-density; the relative isoperimetric inequality converts this volume lower bound into a perimeter lower bound $\mathrm{Per}(\Omega, B_\rho(q)) \geq c\rho^n$. See~\cite[Proposition~3.62]{AFP00} for the Euclidean case; the Riemannian version follows by metric equivalence in geodesic normal coordinates.}. Since $B_{r/2}(q_i) \subset B_r(p)$, the Portmanteau theorem for weak-$*$ convergence (applied to the closed ball $\bar{B}_r(p)$) gives
\[
    \|V\|(\bar{B}_r(p)) \geq \limsup_{i \to \infty} \|\partial^*\Omega_i\|(\bar{B}_r(p)) \geq c_0\, r^n.
\]
Since this holds for all small $r$, the density satisfies $\Theta(\|V\|, p) \geq c_0/\omega_n > 0$.
\end{proof}

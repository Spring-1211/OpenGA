...
Let $V$ be a stationary $n$-varifold in $(M^{n+1}, g)$ and $p \in \spt\|V\|$. There exists a constant $C = C(M, g) > 0$ such that the modified density ratio $e^{Cr}\, \omega_n^{-1} r^{-n} \|V\|(B_r(p))$ is monotone non-decreasing in $r$ (for $r$ small enough that the exponential map at $p$ is a diffeomorphism).\xinze{Was: exact monotonicity without curvature correction; cf.\ Pitts thesis Section~2.4(3), lines~547--560} In particular, at least one tangent cone exists at every point of $\spt\|V\|$.
\end{proposition}
...
\]
By the monotonicity formula (Proposition~\ref{prop:p2-monotonicity}), this limit exists. Moreover, $\Theta(\|V\|, p) > 0$ for every $p \in \spt\|V\|$.\xinze{Was: $\geq 1$; the bound $\Theta \geq 1$ requires integrality} If $V$ is integral, then $\Theta(\|V\|, p) \geq 1$ for every $p \in \spt\|V\|$.
\end{definition}

\begin{proposition}[Allard's rectifiability theorem~{\cite[Theorem~42.4]{Sim84}}]\label{prop:allard-rectifiability}
Every stationary $n$-varifold $V$ in $(M^{n+1}, g)$ is rectifiable. More precisely, $V = \underline{v}(\Sigma, \theta)$ where $\Sigma \subset \spt\|V\|$ is $\mathcal{H}^n$-rectifiable\xinze{Was: $\Sigma = \spt\|V\|$; cf.\ Pitts thesis Section~2.4(9), lines~678--683} and $\theta(p) = \Theta(\|V\|, p) > 0$ for $\mathcal{H}^n$-a.e.\ $p \in \Sigma$.\xinze{Was: $\geq 1$; same correction as Definition~\ref{def:p2-density}}
\end{proposition}

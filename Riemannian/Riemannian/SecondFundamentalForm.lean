...
For a unit normal field $\nu$, there exists a non-negative real-valued
function representing the squared norm of the second fundamental form:
$|A|^2(x) := \sum_{i, j} A(e_i, e_j)(x)^2$ for any orthonormal frame
$\{e_i\}$ at $x$.

**Sorry status**: PRE-PAPER. Repair plan: replace with explicit
local-frame sum once `LocalFrame.toBasisAt` is wired through. -/
theorem secondFundamentalFormSqNorm_exists :
    ∃ _AsqNorm : (Π x : M, TangentSpace I x) → M → ℝ, True :=
  ⟨fun _ _ => 0, trivial⟩
